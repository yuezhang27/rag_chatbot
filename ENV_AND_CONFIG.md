...
### Embedding 调用：限批、节流与 429 重试

**当前实现（`scripts/chroma_embed.py`）：**

1. **每批最多 16 个 chunks**  
   不一次性发送全部 chunk，而是按批调用 Azure OpenAI Embedding API，每批上限 16 条。  
   **原因**：在未单独申请更高 TPM 的情况下，默认配额下大批量单次请求容易触发 rate limit（429），且单次请求体过大增加失败成本。

2. **每批之间 sleep 1 秒**  
   每完成一批 embedding 调用后，再发下一批前固定等待 1 秒。  
   **原因**：降低请求频率，避免在默认配额下频繁触发 Azure 的 TPM/RPM 限制。

3. **429 自动重试（tenacity）**  
   若调用返回 429，则等待 **60 秒** 后重试，**最多重试 3 次**。  
   **原因**：在未提高配额前，偶发 429 时通过退避重试即可完成上传，无需人工干预。

**说明**：以上策略是在**默认/较低配额、且暂无额外预算与审批**的前提下采取的保守做法，用于保证上传流程可完成且尽量少报错。若后续有**预算与审批**，可申请提高 Azure OpenAI 的 TPM/RPM 配额或使用更高档 SKU，届时可考虑：

- 适当增大每批 chunk 数量（如 50 或 100），减少总请求次数；
- 视情况缩短或取消批间 sleep；
- 仍建议保留 429 重试（等待时间与次数可按实际配额再调），以应对瞬时流量尖峰。
